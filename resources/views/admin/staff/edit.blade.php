        ...
        <form action="{{ route('staff.update', $staff) }}" method="POST">
                        ...
                        <div class="ms-2">
                            @foreach($perms as $key => $label)
                                <div class="form-check">
                                    <input class="form-check-input group-item" type="checkbox" name="permissions[]" value="{{ $key }}" id="perm_{{ $loop->parent->index }}_{{ $loop->index }}" {{ in_array($key, old('permissions', $staffPermissions)) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ old('status', $staff->status) ? 'checked' : '' }}>
